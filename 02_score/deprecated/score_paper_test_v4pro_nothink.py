#!/usr/bin/env python3
"""DeepSeek V4-Pro 不思考 打分测试 — 并发版

用法: uv run scripts/score_paper_test_v4pro_nothink.py [N] [model] [workers]
默认 N=20, model=deepseek-v4-pro, workers=16

特性:
- 并发请求(ThreadPoolExecutor),墙钟时长降到 ~ ceil(N / workers) × 单篇耗时
- 实时落盘:每篇完成立即追加到输出 JSONL,中断不丢已完成数据
- 续跑:启动时扫描已有输出,跳过已完成的 paper_id
- 关闭思考模式(与 flash 不思考对照)
...
OUTPUT_PATH = "data/llm_test_v4pro_nothink_run.jsonl"
...
def main():
    ...
    print(f"模型: {model}(不思考)  并发: {workers}  prompt: {len(system_prompt)} 字符")
    print(
        ...
        with tqdm(total=len(todo), desc="打分中", unit="篇") as pbar:
            for fut in as_completed(futures):
                ...
                if rec.get("ok"):
                    ...
                    pbar.set_postfix(
                        ok=ok_count, fail=fail_count, last=f"rigor={mr} integ={integ}"
                    )
                else:
                    fail_count += 1
                    pbar.set_postfix(
    ...
    with open(OUTPUT_PATH, encoding="utf-8") as f:
        for line in f:
            try:
                ...
                continue
    if n_with_usage:
        print(f"输入 tokens: {total_in:,}  输出 tokens: {total_out:,}")
    print(f"→ {OUTPUT_PATH}")
