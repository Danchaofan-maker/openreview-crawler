#!/usr/bin/env python3
"""DeepSeek V4-Flash 全量打分脚本 — 直接读 data/raw/*.jsonl,用 prompt.md 给所有论文打分

用法:
  uv run scripts/score_paper_full_v4flash.py [workers] [model] [max_papers]

默认:
  workers=128, model=deepseek-v4-flash, max_papers=∞

特性:
  - 读 data/raw/*.jsonl 全量论文(自动去重),排除 data/excluded/
  - 实时落盘到 data/llm_full_v4flash_run.jsonl(append + 锁,中断不丢)
  - 续跑:启动时扫描已完成的 paper_id,只跑剩余
  - tqdm 进度条 + 每分钟 heartbeat 统计(token 用量、估算成本、ETA)
  - 失败处理:HTTP 错误指数退避重试 3 次,最终失败写入 data/llm_full_v4flash_failures.jsonl
...
# ---------- 配置 ----------
PROMPT_PATH = "deprecated_prompt_flash.md"
RAW_DIR = "data/raw"
OUTPUT_PATH = "data/llm_full_v4flash_run.jsonl"
FAILURE_PATH = "data/llm_full_v4flash_failures.jsonl"
LOG_PATH = "data/llm_full_v4flash_run.log"
API_URL = "https://api.deepseek.com/v1/chat/completions"

# 价格(v4-flash,USD per 1M tokens)
PRICE_INPUT = 0.055
PRICE_OUTPUT = 0.29
...
def estimate_cost(in_tok: int, out_tok: int) -> float:
    return (in_tok * PRICE_INPUT + out_tok * PRICE_OUTPUT) / 1_000_000
...
def update_state(rec: dict):
    with _lock:
        if rec.get("ok"):
            ...
            _state["cost_usd"] += estimate_cost(
                rec.get("tokens_in") or 0, rec.get("tokens_out") or 0
            )
        else:
            _state["fail"] += 1
...
def main():
    ...
    log(f"=== 完成 ===")
    log(
        f"总用时 {timedelta(seconds=int(elapsed))}  ok={_state['ok']}  fail={_state['fail']}"
    )
    log(
        f"in={_state['in_tokens']:,}  out={_state['out_tokens']:,}  cost=${_state['cost_usd']:.4f}"
    )
